...
int validate_signature_secp256r1_raw(void *prefilled_data, const uint8_t *sig,
    ...
    // generate a keypair and sign the message as instructed in
    // ../docs/secp256r1.md The only difference is that the message here is the
    // sha256 hash instead of the message itself. You may download my exported
    // key pair, message, hash, signatures here
